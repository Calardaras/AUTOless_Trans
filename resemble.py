...
key_l = list(key_f)
key_f.close()
contents_f = open( path_tmp + '\\' + 'contents.txt' ,'r',encoding="utf-8") 
...
# for content in contents_f:
#     curr_f = open( path_otp  + '\\' + re.search('.+(?=(_l_english))',re.search('^[^,]+', content).group(0)).group(0) + '_l_simp_chinese.yml','w+',encoding="utf-8") 
